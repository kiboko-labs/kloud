...
final class OroPlatform implements PHPVersionConsoleDelegatedGuesserInterface
{
    const CODE = 'oroplatform';
    ...
    public function guess(InputInterface $input, OutputInterface $output, Stack\DTO\Context $context): Stack\DTO\Context
    {
        ...
        if ($context->isEnterpriseEdition && Semver::satisfies($context->phpVersion, '>=5.6 <7.1')) {
            $context->applicationVersion = $format->askQuestion(
                (new ChoiceQuestion('Which OroPlatform Enterprise version are you using?', ['1.8', '1.10', '1.12', '2.6'], '2.6'))
            );
        } else if (!$context->isEnterpriseEdition && Semver::satisfies($context->phpVersion, '>=5.6 <7.1')) {
            $context->applicationVersion = $format->askQuestion(
                (new ChoiceQuestion('Which OroPlatform Community version are you using?', ['1.6', '1.8', '1.10', '2.6'], '2.6'))
            );
            ...
            $context->applicationVersion = $format->askQuestion(
                (new ChoiceQuestion('Which OroPlatform version are you using?', ['3.1', '4.1'], '4.1'))
            );
        } else if (Semver::satisfies($context->phpVersion, '>=7.4')) {
            $context->applicationVersion = $format->askQuestion(
                (new ChoiceQuestion('Which OroPlatform version are you using?', ['4.1', '4.2', '5.0'], '4.2'))
            );
        } else if (Semver::satisfies($context->phpVersion, '>=8.0')) {
            $format->writeln(' <fg=green>Choosing automaticallly OroPlatform version 4.2.</>');
            $context->applicationVersion = '4.2';
        } else if (Semver::satisfies($context->phpVersion, '>=8.1')) {
            $format->writeln(' <fg=green>Choosing automaticallly OroPlatform version 5.0.</>');
