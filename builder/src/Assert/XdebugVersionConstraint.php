...
final class XdebugVersionConstraint implements AssertionInterface
{
    ...
    {
        return sprintf('Version for Xdebug PHP extension does not match constraint "%s".', $this->constraint);
    }
    ...
    public function __invoke(): bool
    {
        ...
        $process->run(function ($type, $buffer) use ($process, &$version) {
            ...
            if (preg_match('/Version\s*=>\s*(\d+\.\d+\.\d+(?:[\.-](?:alpha|beta|rc)\d+)?)/i', $buffer, $matches)) {
                $version = $matches[1];
            }
        });
